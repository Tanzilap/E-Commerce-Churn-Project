...
if uploaded_file:
    ...
    filtered_df["month"] = filtered_df["invoice_date"].dt.to_period("M").astype(str)  # Convert invoice date to month
    sales_trend = filtered_df.groupby("month")["price"].sum().reset_index()
    ...
    age_distribution = filtered_df["age"].value_counts().reset_index()
    ...
    top_products = filtered_df.groupby("item")["price"].sum().nlargest(10).reset_index()
    ...
    map_df = filtered_df.groupby(["city", "country"], as_index=False)["price"].sum()
    fig = px.scatter_geo(
        map_df, 
        locations="country", 
